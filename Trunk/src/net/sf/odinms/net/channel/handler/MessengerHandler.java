		...
		switch (mode) {
			...
			case 0x03: // invite
                                if (messenger.getMembers().size() < 3) {
                                    ...
                                    if (target != null) {
                                            if(target.getGMLevel() > 1 && (target.getGMLevel() > player.getGMLevel())){
                                                c.getSession().write(MaplePacketCreator.getWhisperReply(target.getName(), (byte) 0));
                                            } else if (target.getMessenger() == null){

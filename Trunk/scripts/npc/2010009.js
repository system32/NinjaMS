    ...
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			if (selection == 0) {
				...
				if (cm.getPlayer().getParty() == null) {
					cm.sendNext("You may not extend an alliance until you get into a party of 2 people"); //Not real text
					cm.dispose();
				} else if (partymembers.get(0).getGuild() == null) {
					cm.sendNext("You cannot form a Guild Union until own have a guild");
					cm.dispose();
					...
					cm.sendNext("Get your other party member on the same map please.");
					cm.dispose();
				} else if (cm.getPlayer().getMGC().getAllianceRank() != 1) {
					cm.sendNext("Only the Guild Union Master can expand the number of guilds in the Union.");
					cm.dispose();
				} else if (partymembers.get(0).getGuildRank() != 1 || partymembers.get(0).getGuildRank() != 1) {
					cm.sendNext("You must be guild leaders to continue.");
					cm.dispose();
				} else {
					cm.dispose();
					cm.sendOk("Processing...");
					cm.addPlayerGuildToAlliance();
				}
			} else if(selection == 4) {
